...
**Current state:** build 38 — Phase A complete + i18n + visual refresh + tests + speaking-first "Üben" + one-tap reveal + **Sprint** (60-sec hands-free fluency round) + **speaking scoreboard** (Profile "Sprechen" section: words spoken today, 7-day chart, Sprint best, fluency trend) + **Settings re-skin** (on-brand cream surface + card rows + featured language row — no longer a system-grey "bolt-on" Form), on TestFlight (personal).

> **Engagement push (post-Phase-A, in progress):** making practice less monotonous and more effective via *exercise variety* — the genuinely-fun, non-manipulative half of what Duolingo does (we still refuse mascots/hearts/streak-panic; see below). Build 30 added the **"Wählen"** multiple-choice mode. Build 31 made the reveal **encouraging** (growth-mindset: "Noch nicht", not "Falsch"). Build 32 turned the default mode into **"Üben"** — a *smart mix* that varies the exercise by card maturity. Build 33 makes it **speaking-first** (per user preference): new → multiple-choice recognition, mature → **speak it** (on-device `ru-RU` ASR), zero keyboard. "Tippen" kept as an explicit typing drill. Decided to mix within "Üben" rather than add modes. Build 34 adds an **"Ich kann gerade nicht sprechen"** escape hatch — pauses speaking for the session and falls back to keyboard-free multiple-choice on the *same* schedule, with a "Wieder sprechen" banner to resume. **Next:** auto-fallback if mic/speech permission is denied (vs. the manual toggle); **tap-the-tiles** in the learning stage (new → choice → tiles → speak); richer feedback (sound/combo); then topics/lectures as a *journey* (tutor-import differentiator).
>
> **Speaking-volume thread (build 36+, new):** Alex's thesis — *"I learn best when forced to speak a lot"* — opens a parallel push: make spoken-output **volume** a first-class, celebrated metric (not just recall accuracy). Build 36 ships **"Sprint"** — a 60-second hands-free fluency round: German prompts come fast, you say the translation aloud, on-device ASR auto-advances on a close-enough match; scored on cards-cleared (volume, not accuracy); *outside* the FSRS schedule; personal-best via `@AppStorage`. Entry = ⚡ in the Practice header (placement provisional — a session-summary "fast round?" entry is the better home). Match logic = `SprintMatcher` (unit-tested); live mic loop needs device testing. Build 37 ships the **scoreboard reframe** — a Profile "Sprechen" section (words spoken today + 7-day chart + Sprint best + a fluency/response-time trend), all derived from existing reviews + the Sprint tally, no schema migration. **Next on this thread:** **"Gespräch"** — an on-device conversation partner (reuse the `FoundationModelJudge` `LanguageModelSession` as a roleplay interlocutor; iOS 26-gated, vocab-scoped); plus a daily *spoken-words goal* and richer fluency curve (persist the `startDelaySec`/`longestPauseSec` hesitancy signal, currently only used to nudge a grade). See the on-device, no-dark-patterns constraints below.
